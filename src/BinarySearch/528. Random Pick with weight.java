...
// Methodology:
// Use prefixSum[] array and totalSum to caculate the sum of the elements in the giving array.
// Use Math.random() to calcuate the target.  ---> target = Math.random() * totalSum
// For each pickIndex() operation, we need to leverage the binary Search algo.

// Thing worth attention:
// target is a DOUBLE
// Binary Search should be correct, if the prefixSum[mid] > target
// then we should move the mid one index advance along the array.

class Solution {
    // Methodology
    // Use binarySearch to find the index
    ...
    public int pickIndex() {
        ...
        while (low < high) {
            ...
            // the key is to compare target with prefixSum[mid]
